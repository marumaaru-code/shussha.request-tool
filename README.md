...
- **座席残数の目安（曜日別）**は、フォームの下に**常に表示**されます（誰でも見られます）。依頼する前に空いている曜日を確認できます。
- **インターン生**：所属（SNS / LINE / AI / 広告 / 制作）を選び、SNSならunit A〜H・広告ならunit A〜Dを選択（LINE・AI・制作はunitの選択なし）。あとは名前・出社したい日付・（任意で）理由を入れて「提出する」を押すだけ。
- **確認する人（承認側）**：ヘッダー右上の「確認者」を押して**確認者モード**をONにします。**パスワード**（`CONFIG.REVIEWER_PASSWORD`）を求められ、合致するとONになります（その端末では記憶。OFF→再ONで再度要求）。ONにすると：
  - 各依頼カードにも「その曜日の2F残席」が出るので、空きを見ながら承認できます。
  ...
  - 各依頼に「**確認者コメント**」欄があり、記入して「コメント保存」を押すと保存されます。このコメントはインターン側のカードにも「💬 確認者コメント」として表示されます。
  - 「承認待ち / 承認済み / すべて」で絞り込めます。
- インターン生側は、自分の依頼に「承認待ち・承認済み・却下」のバッジが付くので、結果がひと目で分かります（承認/却下・削除・コメントなどの確認者用ボタンは、モードOFFのときは出ません）。
- 提出・承認はリアルタイムで全員に反映されます。

### 出社管理タブ（誰がいつ出社するか・出社回数）

- 上部の「**依頼 / 出社管理**」タブは、**確認者以外（インターン）も見られます**（読み取り専用）。誰がどの日に出社するか、みんなで軽く確認できます。
- 2つの表示を切り替えられます：
  - **日付ごと**（初期表示）：どの日に誰が出社するかを表示。**未来を昇順（近い日が上）→ 過去（新しい順）**で並びます。**承認待ちの依頼も含み**、承認前のものには「承認待ち」バッジが付きます（却下は除外）。「◯人」と unit・名前が並びます。
  - **人ごと（月別）**：**承認済み**の別曜日出社を月ごと・人ごとに集計（＝実際の出社回数）。各人の「◯回」と出社日が並び、月ごとに「◯人 / のべ◯回」も出ます。
